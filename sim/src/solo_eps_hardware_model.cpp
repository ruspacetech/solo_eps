#include <solo_eps_hardware_model.hpp>

namespace Nos3
{
    REGISTER_HARDWARE_MODEL(Solo_epsHardwareModel,"SOLO_EPS");

    extern ItcLogger::Logger *sim_logger;

    const std::string Solo_epsHardwareModel::_solo_eps_stream_name = "solo_eps_stream";

    Solo_epsHardwareModel::Solo_epsHardwareModel(const boost::property_tree::ptree& config) : SimIHardwareModel(config), _stream_counter(0)
    {
        ...
        sim_logger->info("Solo_epsHardwareModel::Solo_epsHardwareModel:  NOS Engine connection string: %s.", connection_string.c_str());

        /* vvv 1. Get a data provider */
        /* !!! If your sim does not *need* a data provider, delete this block. */
        std::string dp_name = config.get("simulator.hardware-model.data-provider.type", "SOLO_EPS_PROVIDER");
        _solo_eps_dp = SimDataProviderFactory::Instance().Create(dp_name, config);
        sim_logger->info("Solo_epsHardwareModel::Solo_epsHardwareModel:  Data provider %s created.", dp_name.c_str());
        ...
        _uart_connection.reset(new NosEngine::Uart::Uart(_hub, config.get("simulator.name", "solo_eps_sim"), connection_string, bus_name));
        _uart_connection->open(node_port);
        sim_logger->info("Solo_epsHardwareModel::Solo_epsHardwareModel:  Now on UART bus name %s, port %d.", bus_name.c_str(), node_port);
        ...
        _uart_connection->set_read_callback(std::bind(&Solo_epsHardwareModel::uart_read_callback, this, std::placeholders::_1, std::placeholders::_2));
        ...
        _streaming_data_function_map.insert(std::map<std::string, streaming_data_func>::value_type(_solo_eps_stream_name, &Solo_epsHardwareModel::create_solo_eps_data));
        ...
        if (config.get_child_optional("simulator.hardware-model.default-streams")) 
        {
            BOOST_FOREACH(const boost::property_tree::ptree::value_type &v, config.get_child("simulator.hardware-model.default-streams")) // Loop through the default streams for *this* hw model
            {
                ...
                if ((_streaming_data_function_map.find(stream_name) != _streaming_data_function_map.end()) &&
                    ...
                    sim_logger->info("Solo_epsHardwareModel::Solo_epsHardwareModel:  Created default stream name %s starting at %f (start time + %f) with stream period %d milliseconds.", 
                        stream_name.c_str(), _absolute_start_time + initial_stream_time, initial_stream_time, stream_period_ms);
                } else {
                    sim_logger->error("Solo_epsHardwareModel::Solo_epsHardwareModel:  Invalid stream name %s or stream period (must be > 0) %d.", 
        ...
        _time_bus->add_time_tick_callback(std::bind(&Solo_epsHardwareModel::send_streaming_data, this, std::placeholders::_1));
        sim_logger->info("Solo_epsHardwareModel::Solo_epsHardwareModel:  Now on time bus %s, executing callback to stream data.", time_bus_name.c_str());
        /* ^^^ 3. Streaming data */
    }

    // vvv Pretty standard... only change me if a different bus type is used and/or the data provider is not needed
    Solo_epsHardwareModel::~Solo_epsHardwareModel(void)
    {        
        // 1. Close the uart
        ...
        delete _solo_eps_dp;
        _solo_eps_dp = nullptr;

        // 3. Don't need to clean up the time node, the bus will do it
    }

    // vvv Automagically set up by the base class to be called
    void Solo_epsHardwareModel::command_callback(NosEngine::Common::Message msg)
    {
        // Here's how to get the data out of the message
        NosEngine::Common::DataBufferOverlay dbf(const_cast<NosEngine::Utility::Buffer&>(msg.buffer));
        sim_logger->info("Solo_epsHardwareModel::command_callback:  Received command: %s.", dbf.data);

        // Do something with the data
        std::string command = dbf.data;
        std::string response = "Solo_epsHardwareModel::command_callback:  INVALID COMMAND! (Try STOP SOLO_EPS)";
        boost::to_upper(command);
        if (command.compare("STOP SOLO_EPS") == 0) 
        {
            _keep_running = false;
            response = "Solo_epsHardwareModel::command_callback:  STOPPING SOLO_EPS";
        }
    ...
    void Solo_epsHardwareModel::send_streaming_data(NosEngine::Common::SimTime time)
    {
        const boost::shared_ptr<Solo_epsDataPoint> data_point =
            boost::dynamic_pointer_cast<Solo_epsDataPoint>(_solo_eps_dp->get_data_point());
    ...
    void Solo_epsHardwareModel::create_solo_eps_data(const Solo_epsDataPoint& data_point, std::vector<uint8_t>& out_data)
    {
        ...
        // so that's what this solo_eps is going to do... so scale each of the x, y, z (which are in the range [-1.0, 1.0]) by 32767
        ...
        // ... you can check this value and make a decision:  data_point.is_solo_eps_data_valid()
        ...
        uint16_t x   = (uint16_t)(data_point.get_solo_eps_data_x()*32767.0 + 32768.0);
        out_data[6]  = (x >> 8) & 0x00FF;
        out_data[7]  =  x       & 0x00FF;
        uint16_t y   = (uint16_t)(data_point.get_solo_eps_data_y()*32767.0 + 32768.0);
        out_data[8]  = (y >> 8) & 0x00FF;
        out_data[9]  =  y       & 0x00FF;
        uint16_t z   = (uint16_t)(data_point.get_solo_eps_data_z()*32767.0 + 32768.0);
    ...
    void Solo_epsHardwareModel::uart_read_callback(const uint8_t *buf, size_t len)
    {
        // Retrieve data and log received data in man readable format
        boost::shared_ptr<Solo_epsDataPoint> data_point;
        std::vector<uint8_t> in_data(buf, buf + len);
        sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  REQUEST %s",
        ...
        if (in_data.size() != 13)
        {
            sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  Invalid command size of %d received!", in_data.size());
            return;
        }
        ...
        {
            sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  Header incorrect!");
            return;
        }
        ...
        {
            sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  Trailer incorrect!");
            return;
        }
        ...
        {
            case 1:
                sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  Send data command received!");
                data_point = boost::dynamic_pointer_cast<Solo_epsDataPoint>(_solo_eps_dp->get_data_point());
                sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  Data point:  %s", data_point->to_string().c_str());
                create_solo_eps_data(*data_point, out_data); // Command not echoed back... actual solo_eps data is sent
                break;

            case 2:
                sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  Configuration command received!");
                if ((in_data[2] == _solo_eps_stream_name[0]) && 
                    (in_data[3] == _solo_eps_stream_name[1]) && 
                    (in_data[4] == _solo_eps_stream_name[2]) && 
                    (in_data[5] == _solo_eps_stream_name[3])) { 
                    // ... this is a good example of the type of thinking you need to do in the hardware model to make its byte interface behave
                    ...
                    uint32_t millisecond_stream_delay = ((uint32_t)in_data[7] << 24) +
                                                        ...
                                                        ((uint32_t)in_data[10]);
                    std::map<std::string, boost::tuple<double, double>>::iterator it = _periodic_streams.find(_solo_eps_stream_name);
                    if ((it != _periodic_streams.end()) &&
                        (millisecond_stream_delay > 0)) {
                        boost::get<1>(it->second) = ((double)millisecond_stream_delay)/1000.0;
                        sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  New millisecond stream delay for %s of %u", 
                            _solo_eps_stream_name.c_str(), millisecond_stream_delay);
                    } else {
                        sim_logger->error("Solo_epsHardwareModel::uart_read_callback:  Stream %s was not set to be executed periodically or delay %u was not > 0",
                            _solo_eps_stream_name.c_str(), millisecond_stream_delay);
                    ...
                    sim_logger->error("Solo_epsHardwareModel::uart_read_callback:  Requested stream %c%c%c%c does not match prefix of %s", 
                        in_data[3], in_data[4], in_data[5], in_data[6], _solo_eps_stream_name.c_str());
                    // zero out the response data... to indicate invalid request
                    in_data[ 7] = 0;
                    in_data[ 8] = 0;
                    in_data[ 9] = 0;
            ...
            case 3:
                sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  Other command received!");
                break;
            
            default:
                sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  Unused command received!");
                break;
        }

        // Log reply data in man readable format and ship the message bytes off
        sim_logger->debug("Solo_epsHardwareModel::uart_read_callback:  REPLY %s",
