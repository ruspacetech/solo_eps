#ifndef NOS3_SOLO_EPSHARDWAREMODEL_HPP
#define NOS3_SOLO_EPSHARDWAREMODEL_HPP
...
namespace Nos3
{
    ...
    class Solo_epsHardwareModel : public SimIHardwareModel
    {
    public:
        // Constructors / destructor
        Solo_epsHardwareModel(const boost::property_tree::ptree& config);
        ~Solo_epsHardwareModel(void);
    ...
    private:
        ...
        void create_solo_eps_data(const Solo_epsDataPoint& data_point, std::vector<uint8_t>& out_data); // This guy creates data to send from a data point
        ...
        SimIDataProvider*                                   _solo_eps_dp; // I'm only needed if the sim actually has/needs a data provider

        // vvv Standard maps needed to set up streaming
        typedef void (Solo_epsHardwareModel::*streaming_data_func)(const Solo_epsDataPoint&, std::vector<uint8_t>&); // Convenience pointer to function typedef
        ...
        static const std::string                            _solo_eps_stream_name; // Used in this example to validate commands sent over UART to this hardware model sim
    };
}
